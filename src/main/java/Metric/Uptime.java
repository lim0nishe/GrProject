...
public class Uptime implements Metric {
    ...
    private static final int TEST_UPTIME_CRITICAL_VALUE = 100;
    ...
    public Double getParameter(FTPServer server, String ftpProperties, String quota) {
        try {
            ...
            Double result = Math.exp( - Double.parseDouble(uptimeData[0] + TEST_UPTIME_CRITICAL_VALUE));
            logger.info("uptime metric for server " + server.getId() + " : " + result);
            connectionController.closeSession();
            return result;
            //return Math.exp( - (Double.parseDouble(uptimeData[0]) / SECONDS_IN_DAY) + MINIMUM_UPTIME_DAYS_NUMBER);
        }
        catch (Exception e1){
            e1.printStackTrace();
